...
$iniPath = ( $siteAccess == "global_override" ) ? "extension/debug/settings" : "extension/debug/settings/siteaccess/$siteAccess";
...
foreach( $iniFiles as $fileName => $settings )
{
    ...
    {
        $ini = eZINI::instance( $fileName . '.append', "extension/debug/settings", null, null, null, true, true );
        foreach( $settings as $setting )
        {
            if ( $ini->hasVariable( $setting[0], $setting[1] ) )
